...
UPLOAD_TOKEN_MAX_LIFETIME_SECONDS = (
    UPLOAD_TOKEN_MAX_LIFETIME_HOURS * 3600
)  # 1,728,000 seconds
...
def hash_password(password: str) -> Tuple[str, str]:
    ...
    try:
        ...
        raise ValueError(
            f"Failed to generate cryptographically secure salt: {type(e).__name__}"
        )
...
def verify_password(
    plain_password: str, hashed_password: str, salt: str = None, user_id: str = None
) -> bool:
    ...
    try:
        if not plain_password or not hashed_password:
            _log(
                "debug",
                f"Password verification failed: missing input (user: {user_id})",
            )
            return False
        ...
        if salt is None:
            ...
            if "$" in hashed_password:
                ...
                if len(parts) != 2:
                    _log(
                        "warning",
                        f"Invalid hash format: expected 2 parts, got {len(parts)} (user: {user_id})",
                    )
                    return False
                ...
                if not salt or not stored_hash:
                    _log(
                        "warning",
                        f"Invalid hash format: empty salt or hash (user: {user_id})",
                    )
                    return False
                ...
                if len(salt) != 32:
                    _log(
                        "warning",
                        f"Invalid salt length: expected 32, got {len(salt)} (user: {user_id})",
                    )
                    return False
                ...
                try:
                    ...
                    password_hash = hashlib.pbkdf2_hmac(
                        "sha256", password_bytes, salt.encode("utf-8"), 100000
                    )
                ...
                return _verify_legacy_passwords(
                    plain_password, hashed_password, salt, user_id
                )
        else:
            ...
            if len(plain_password) > 128:
                _log(
                    "warning",
                    f"Password verification failed: password exceeds maximum length (user: {user_id})",
                )
                return False
            ...
            if len(hashed_password) > 256:
                _log(
                    "warning",
                    f"Password verification failed: hash exceeds maximum length (user: {user_id})",
                )
                return False
            ...
            try:
                ...
                if not is_valid and salt:
                    _log(
                        "debug",
                        f"PBKDF2 with provided salt failed, trying legacy SHA256+salt format (user: {user_id})",
                    )
                    legacy_result = _verify_legacy_passwords(
                        plain_password, hashed_password, salt, user_id
                    )
                    if legacy_result:
                        _log(
                            "warning",
                            f"User {user_id} using legacy SHA256+salt format - migration recommended",
                        )
                        return True
                    else:
                        _log(
                            "error",
                            f"Legacy SHA256+salt verification also failed for {user_id}",
                        )
                        return False
                ...
                if (
                    ...
                    and all(c in "0123456789abcdefABCDEF" for c in hashed_password)
                ):
                    _log(
                        "debug",
                        f"PBKDF2 with provided salt failed, trying legacy SHA256 (user: {user_id})",
                    )
                    ...
                    if is_valid:
                        _log(
                            "warning",
                            f"User {user_id} using legacy SHA256 password - migration recommended",
                        )
...
def _verify_legacy_passwords(
    ...
    try:
        ...
        if salt and hashed_password:
            _log(
                "debug",
                f"Attempting legacy password verification formats for {user_id}",
            )
            ...
            try:
                ...
                if hmac.compare_digest(legacy_hash, hashed_password):
                    _log(
                        "warning",
                        f"User {user_id} using legacy SHA256(password+salt) format - migration recommended",
                    )
                    return True

                # Format 2: SHA256(salt + password) with hex salt
                combined_input_alt = salt + plain_password
                legacy_hash_alt = hashlib.sha256(
                    combined_input_alt.encode()
                ).hexdigest()
                if hmac.compare_digest(legacy_hash_alt, hashed_password):
                    _log(
                        "warning",
                        f"User {user_id} using legacy SHA256(salt+password) format - migration recommended",
                    )
                    return True
                ...
                try:
                    ...
                    if hmac.compare_digest(pbkdf2_hex, hashed_password):
                        _log(
                            "warning",
                            f"User {user_id} using legacy PBKDF2 with string salt - migration recommended",
                        )
                        return True
                except Exception as e:
                ...
                if len(salt) == 32 and all(c in "0123456789abcdefABCDEF" for c in salt):
                    try:
                        ...
                        if hmac.compare_digest(pbkdf2_hex, hashed_password):
                            _log(
                                "warning",
                                f"User {user_id} using legacy PBKDF2 with hex salt - migration recommended",
                            )
                            return True
                    except Exception as e:
                ...
                _log(
                    "error",
                    f"Legacy SHA256+salt verification failed: {type(e).__name__}",
                )
                ...
                if len(salt) == 32 and len(stored_hash) == 64:
                    try:
                        ...
                        if is_valid:
                            _log(
                                "warning",
                                f"User {user_id} using legacy password format - migration recommended",
                            )
                        return is_valid
                    except Exception as e:
                        _log(
                            "error",
                            f"Legacy hash verification failed: {type(e).__name__}",
                        )
                        return False

        # Original checks for pure SHA256 (64 chars) and MD5 (32 chars)
        if len(hashed_password) == 64 and all(
            c in "0123456789abcdefABCDEF" for c in hashed_password
        ):
            ...
            try:
                ...
                if hmac.compare_digest(legacy_hash, hashed_password):
                    _log(
                        "warning",
                        f"User {user_id} using legacy password hash - migration recommended",
                    )
                    return True
                return False
            except Exception as e:
            ...
            try:
                ...
                if hmac.compare_digest(md5_hash, hashed_password):
                    _log(
                        "warning",
                        f"User {user_id} using INSECURE MD5 password hash - migration required",
                    )
                    return True
                return False
            except Exception as e:
            ...
            _log(
                "warning",
                f"Invalid password hash format for user {user_id}: length={len(hashed_password)}, format={hashed_password[:20]}...",
            )
            return False
    ...
    if hashed_password:
        ...
        if "$" in hashed_password:
            ...
            if len(parts) == 2:
                diagnosis["hash"][
                    "details"
                ] = f"2 parts: {len(parts[0])}-char salt, {len(parts[1])}-char hash"
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    ...
    if to_encode.get("token_type") in ["access", "refresh"] and not to_encode.get(
        "jti"
    ):
        to_encode["jti"] = str(uuid.uuid4())
...
def decode_token(token: str) -> TokenData:
    ...
    try:
        ...
        jti: Optional[str] = payload.get(
            "jti"
        )  # CRITICAL FIX: Extract JTI for token revocation
        ...
        if not (
            ...
            or (
                len(user_id) > 0 and user_id.replace("_", "").replace("-", "").isalnum()
            )
        ):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        if not token_type or token_type not in [
            "access",
            "refresh",
            "password_reset",
            "upload",
        ...
        if token_type == "upload":
            ...
            if (
                exp_timestamp - issued_at > UPLOAD_TOKEN_MAX_LIFETIME_SECONDS
            ):  # More than 480 hours
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        return TokenData(
            user_id=user_id, token_type=token_type, jti=jti, payload=payload
        )
...
async def get_current_user_or_query(
    ...
    if token is not None:
        logger.warning(
            "SECURITY VIOLATION: Query parameter authentication attempted - ignored for security"
        )
...
async def get_current_user_for_upload(
    ...
    if not access_token:
        ...
        # SECURITY: Reject query parameter tokens as security violation (consistent with get_current_user)
            ...
            logger.warning(
                "SECURITY VIOLATION: Query parameter authentication attempted in get_current_user_or_query - ignored for security"
            )
    ...
    if not access_token:
        logger.debug(
            "No token string extracted from cookie or auth header - allowing anonymous access"
        )
        return None
    ...
    is_upload_operation = (
        "/files/" in request_path
        or request_path.startswith("/api/v1/files/")
        or "upload" in request_path.lower()
        or "/chats/" in request_path
        and "/messages" in request_path  # Chat messages endpoints
        or request_path.endswith("/messages")
        and "/chats/" in request_path  # Direct chat messages endpoints
    )
...
def _decode_token_with_480_hour_validation(token_str: str) -> Optional[str]:
    ...
    try:
        ...
        if time_since_issue <= max_lifetime:
            ...
            logger.debug(
                f"Accepting expired token within 480-hour window: issued {iat_time}, expired {exp_time}"
            )
            return user_id
        else:
...
def validate_session_code(provided_code: str, stored_code: str) -> bool:
    ...
    if not provided_code or not isinstance(provided_code, str):
        logger.warning(
            f"Session code validation failed: invalid provided_code type {type(provided_code)}"
        )
        return False

    if not stored_code or not isinstance(stored_code, str):
        logger.warning(
            f"Session code validation failed: invalid stored_code type {type(stored_code)}"
        )
        return False
