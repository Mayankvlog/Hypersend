...
class WebSocketManager:
    ...
    def __new__(cls):
        with cls._new_lock:
            if cls._instance is None:
                ...
                cls._instance.device_manager = None
                cls._instance.delivery_manager = None
                cls._instance.signal_protocol = None
                cls._instance.media_service = None
    ...
    async def initialize(self, redis_client: Optional[Any] = None):
        ...
        async with self._lock:
            ...
            self.message_deduplication: Dict[
                str, float
            ] = {}  # message_hash -> timestamp
        ...
        try:
            ...
            async with self.connection_lock:
                ...
                if old_connection and old_connection.websocket:
                    try:
                        ...
                        logger.info(
                            f"Closed duplicate connection for device {device_id}"
                        )
                    except Exception as e:
            ...
            if self.delivery_manager:
                ...
                ip_address = (
                    remote[0]
                    if (
                        remote and isinstance(remote, (list, tuple)) and len(remote) > 0
                    )
                    else ""
                )
            ...
            logger.info(
                f"Device {device_id} connected (total: {len(self.connections)})"
            )
            ...
            try:
                await asyncio.gather(
                    heartbeat_task, pubsub_task, return_exceptions=True
                )
            except Exception:
                pass

    async def _authenticate_device(
        self, websocket: WebSocketServerProtocol, device_id: str, user_id: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to get user devices: {e} - allowing connection without verification"
                )
                return True
            ...
            try:
                ...
                logger.warning(
                    f"Failed to get device session: {e} - allowing connection"
                )
                return True
            ...
            if (
                not hasattr(device_session, "session_key")
                or device_session.session_key is None
            ):
            ...
            expected_response = self._calculate_challenge_response(
                challenge, device_session.session_key
            )
    ...
    async def _handle_delivery_receipt(
        ...
        if self.delivery_manager:
            success = await self.delivery_manager.mark_message_delivered(
                message_id, connection.device_id
            )

            if success:
                # Update message state
                await self.update_message_state(
                    message_id, connection.device_id, "delivered"
                )
                ...
                logger.info(
                    f"Delivery receipt for message {message_id} from device {connection.device_id}"
                )
    ...
    async def _handle_read_receipt(
        ...
        if self.delivery_manager:
            ...
            if success:
                # Update message state
                await self.update_message_state(
                    message_id, connection.device_id, "read"
                )
                ...
                logger.info(
                    f"Read receipt for message {message_id} from device {connection.device_id}"
                )
    ...
    async def _handle_message_state_update(
        ...
        logger.info(
            f"Message {message_id} state updated to {new_state} by device {connection.device_id}"
        )
    ...
    async def _process_offline_messages(self, connection: WebSocketConnection):
        ...
        try:
            ...
            while True:
                ...
                message_data = await self.redis.brpoplpush(
                    main_queue, processing_queue, timeout=1
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"Sent offline message to device {connection.device_id}"
                    )
                except Exception as e:
            ...
            while True:
                try:
                    ...
                    if device_id not in self.connections:
                        logger.debug(
                            f"[HEARTBEAT] Device {device_id} no longer in connections"
                        )
                        break
                    ...
                    if connection.websocket.closed:
                        logger.debug(
                            f"[HEARTBEAT] WebSocket closed for device {device_id}"
                        )
                        break
                    ...
                    try:
                        ...
                        logger.debug(
                            f"[HEARTBEAT] Sent ping #{heartbeat_count} to device {device_id}"
                        )

                    except asyncio.TimeoutError:
                        logger.warning(
                            f"[HEARTBEAT] Timeout sending ping to device {device_id}"
                        )
                        break
                    ...
                    if connection.last_pong < connection.last_ping:
                        ...
                        if time_since_ping > self.pong_timeout:
                            logger.warning(
                                f"[HEARTBEAT] No pong from device {device_id} (waited {time_since_ping:.1f}s)"
                            )
                    ...
                    logger.debug(
                        f"[HEARTBEAT] Connection closed for device {device_id}"
                    )
                    break
                except asyncio.CancelledError:
                    logger.debug(
                        f"[HEARTBEAT] Heartbeat cancelled for device {device_id}"
                    )
                    break
                except Exception as e:
                    logger.error(
                        f"[HEARTBEAT] Unexpected error for device {device_id}: {type(e).__name__}: {e}"
                    )
                    break

        finally:
            logger.debug(
                f"[HEARTBEAT] Exiting heartbeat loop for device {device_id} (sent {heartbeat_count} pings)"
            )
            # Cleanup happens in handle_connection's finally block

    async def _broadcast_to_user_devices(
        self, user_id: str, exclude_device_id: str, message: Dict[str, Any]
    ):
            ...
            for device in user_devices:
                ...
                if connection:
                    try:
                        ...
                        logger.error(
                            f"Failed to broadcast to device {device.device_id}: {e}"
                        )

    async def _broadcast_to_chat_participants(
        self, chat_id: str, exclude_user_id: str, message: Dict[str, Any]
    ):
        ...
        if members_bytes is not None:
            members_data = (
                members_bytes.decode()
                if isinstance(members_bytes, bytes)
                else members_bytes
            )
    ...
    async def _cleanup_connection(self, device_id: str):
        ...
        async with self.connection_lock:
            if device_id in self.connections:
                ...
                logger.info(
                    f"Cleaned up connection for device {device_id} (total: {len(self.connections)})"
                )
    ...
    def _calculate_challenge_response(self, challenge: str, session_key: str) -> str:
        ...
        hmac_obj = hmac.new(session_key.encode(), challenge.encode(), hashlib.sha256)
        ...
        try:
            if self.redis:
                await self.redis.lpush(
                    f"device_offline_queue:{device_id}", json.dumps(message)
                )
    ...
    async def get_connection_stats(self) -> Dict[str, Any]:
        ...
        async with self.connection_lock:
            ...
            now = time.time()
            durations = [now - conn.connected_at for conn in self.connections.values()]

            avg_duration = sum(durations) / len(durations) if durations else 0

            # Copy connections list for safe iteration outside lock
            connections_list = [conn.to_dict() for conn in self.connections.values()]
    ...
    async def _redis_pubsub_listener(
        self, device_id: str, user_id: str, connection: WebSocketConnection
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"[WS-PUBSUB] Device {device_id} subscribed to {len(channels)} Redis channels"
            )
            ...
            async for message in pubsub.listen():
                ...
                if device_id not in self.connections:
                    logger.debug(
                        f"Device {device_id} disconnected during pubsub listen"
                    )
                    break
                ...
                if message["type"] == "message":
                    try:
                        ...
                        logger.warning(
                            f"[WS-PUBSUB] Failed to send to device {device_id}: {e}"
                        )
            ...
            logger.debug(
                f"[WS-PUBSUB] Pub/Sub listening cancelled for device {device_id}"
            )
        except Exception as e:
            logger.error(
                f"[WS-PUBSUB] Error for device {device_id}: {type(e).__name__}: {e}"
            )
        finally:
    ...
    async def start_global_pubsub(self):
        ...
        if not self.redis:
            logger.error(
                "[REDIS-GLOBAL] CRITICAL: Redis not available - cannot start global pubsub"
            )
        ...
        async def global_redis_subscriber():
            ...
            try:
                logger.info(
                    "[REDIS-GLOBAL] Starting global Redis Pub/Sub subscriber..."
                )
                ...
                while reconnect_count < max_reconnect_attempts:
                    ...
                    try:
                            ...
                            try:
                                if message["type"] in ["message", "pmessage"]:
                                    ...
                                    try:
                                        ...
                                        logger.debug(
                                            f"[REDIS-GLOBAL] {message['type']} on {channel}: {data.get('type', 'unknown')}"
                                        )

                                        # Broadcast to relevant WebSocket connections
                                        await self._broadcast_redis_message(
                                            data, channel
                                        )

                                    except json.JSONDecodeError as e:
                                        logger.error(
                                            f"[REDIS-GLOBAL] JSON parse error: {e}"
                                        )
                                ...
                                logger.error(
                                    f"[REDIS-GLOBAL] Message processing error: {type(e).__name__}: {e}"
                                )
                        ...
                        reconnect_count += 1
                        logger.warning(
                            f"[REDIS-GLOBAL] Connection error (attempt {reconnect_count}/{max_reconnect_attempts}): {e}"
                        )
                        ...
                        if reconnect_count < max_reconnect_attempts:
                            ...
                            logger.error(
                                "[REDIS-GLOBAL] Max reconnection attempts reached - stopping"
                            )
                            break

                    except Exception as e:
                        logger.error(
                            f"[REDIS-GLOBAL] Unexpected error: {type(e).__name__}: {e}"
                        )
                        reconnect_count += 1
                        if reconnect_count < max_reconnect_attempts:
    ...
    async def _broadcast_redis_message(self, data: Dict[str, Any], channel: str):
        ...
        try:
            ...
            if message_type in [
                ...
                if user_id:
                    await self._broadcast_to_user_devices(
                        user_id, data.get("device_id", ""), data
                    )
    ...
    async def send_to_user(self, user_id: str, message: Dict[str, Any]) -> int:
        ...
        try:
            ...
            logger.debug(
                f"Sent message to {success_count}/{len(user_devices)} devices for user {user_id}"
            )
    ...
    async def broadcast(self, message: Dict[str, Any]) -> int:
        ...
        if failed_connections:
            ...
            logger.debug(
                f"Cleaned up {len(failed_connections)} failed connections during broadcast"
            )
...
async def create_websocket_server(
    redis_client: Optional[Any], host: str = "0.0.0.0", port: int = 8001
):
