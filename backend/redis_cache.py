...
if not REDIS_AVAILABLE:
    ...
    if os.getenv("DEBUG", "").lower() in ("true", "1", "yes") or os.getenv(
        "REDIS_DEBUG", ""
    ).lower() in ("true", "1", "yes"):
...
class RedisCache:
    ...
    async def connect(
        ...
        if not REDIS_AVAILABLE:
            ...
            logger.warning(
                "[REDIS] Redis library not installed - using in-memory cache (limited functionality)"
            )
            return False
        ...
        try:
            ...
            if host in ("localhost", "127.0.0.1", "::1"):
                logger.error(
                    f"[REDIS] CRITICAL: Redis host is {host} - production must use docker service name 'redis'"
                )
            ...
            logger.info(
                f"[REDIS] Connecting to Redis at {host}:{port} db={db} with password={'REDACTED' if password else 'NONE'}"
            )
            ...
            try:
                ping_result = await asyncio.wait_for(
                    self.redis_client.ping(), timeout=10
                )
                logger.debug(
                    f"[REDIS] Ping result: {ping_result} (type: {type(ping_result).__name__})"
                )
            except asyncio.TimeoutError as timeout_e:
                logger.error(
                    f"[REDIS] CRITICAL: Ping timeout after 10s for {host}:{port}"
                )
                logger.error(
                    f"[REDIS] This typically means: Redis is not responding, network issue, or firewall blocking"
                )
                raise RuntimeError(
                    f"Redis ping timeout - server not responding: {timeout_e}"
                )
            except Exception as ping_e:
                logger.error(
                    f"[REDIS] CRITICAL: Ping failed: {type(ping_e).__name__}: {ping_e}"
                )
                if "authentication" in str(ping_e).lower() or "WRONGPASS" in str(
                    ping_e
                ):
                    logger.error(
                        f"[REDIS] AUTHENTICATION ERROR: Check REDIS_PASSWORD environment variable"
                    )
            ...
            if ping_result:
                ...
                try:
                    info = await asyncio.wait_for(
                        self.redis_client.info("server"), timeout=5
                    )
                    redis_version = info.get("redis_version", "unknown")
                    redis_mode = info.get("redis_mode", "unknown")
                    logger.info(
                        f"[REDIS] Server version: {redis_version}, mode: {redis_mode}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"[REDIS] Pub/Sub test failed: {type(e).__name__}: {e}"
                    )
                    self.is_connected = False
            ...
            logger.error(
                f"[REDIS] CRITICAL: Failed to connect to {host}:{port}: {type(e).__name__}: {e}"
            )
            import traceback
    ...
    async def set(self, key: str, value: Any, expire_seconds: int = 3600):
        ...
        try:
            ...
            if isinstance(value, tuple):
                logger.error(
                    f"[REDIS] CRITICAL: Cannot store tuple in Redis - converting to list for key {key}"
                )
            ...
            if expire_seconds > 0:
                self.mock_expirations[key] = datetime.now(timezone.utc) + timedelta(
                    seconds=expire_seconds
                )
        except (TypeError, ValueError) as e:
            logger.error(
                f"[REDIS] Failed to serialize value for key {key}: {type(e).__name__}: {e}"
            )
    ...
    async def clear_pattern(self, pattern: str):
        ...
        # Fallback to mock cache
        keys_to_remove = [
            k for k in self.mock_cache.keys() if pattern.replace("*", "") in k
        ]
    ...
    async def ttl(self, key: str) -> int:
        ...
        ttl_seconds = (
            self.mock_expirations[key] - datetime.now(timezone.utc)
        ).total_seconds()
    ...
    async def acquire_lock(
        self, key: str, timeout: int = None, wait_time: int = 10
    ) -> bool:
    ...
    async def publish(self, channel: str, message: Any) -> int:
        ...
        try:
            ...
            if isinstance(message, tuple):
                logger.error(
                    f"[REDIS] CRITICAL: Cannot publish tuple to Redis - converting to list"
                )
            ...
            if isinstance(message, (dict, list, str, int, float, bool, type(None))):
                ...
                logger.warning(
                    f"[REDIS] Non-serializable object type being published: {type(message).__name__}"
                )
                serialized_message = json.dumps(
                    {"_type": type(message).__name__, "value": str(message)},
                    default=str,
                    ensure_ascii=False,
                )
            ...
            if self.is_connected and self.redis_client:
                try:
                    ...
                    logger.error(
                        f"[REDIS] Redis publish error: {type(e).__name__}: {e}"
                    )
                    return 0
        except Exception as e:
            logger.error(
                f"[REDIS] Failed to serialize message for publish: {type(e).__name__}: {e}"
            )
            return 0
    ...
    async def zremrangebyscore(
        ...
        if self.is_connected and self.redis_client:
            try:
                return await self.redis_client.zremrangebyscore(
...
class UserCacheService:
    ...
    async def set_user_profile(
        user_id: str, profile_data: Dict, expire_seconds: int = 3600
    ):
    ...
    @staticmethod
    async def set_user_contacts(
        user_id: str, contacts: List, expire_seconds: int = 1800
    ):
...
class GroupCacheService:
    ...
    async def set_group_members(
        group_id: str, members: List, expire_seconds: int = 1800
    ):
    ...
    async def set_group_info(
        group_id: str, group_data: Dict, expire_seconds: int = 1800
    ):
...
class SearchCacheService:
    ...
    @staticmethod
    async def set_contact_suggestions(
        user_id: str, suggestions: List, expire_seconds: int = 600
    ):
...
class SessionCacheService:
    ...
    async def set_session(
        session_id: str, session_data: Dict, expire_seconds: int = 86400
    ):
...
class MessageCacheService:
    ...
    async def add_chat_message(chat_id: str, message: Dict, max_messages: int = 1000):
        ...
        logger.debug(
            f"Message stored in {key} (sorted set) with per-message TTL {ttl_seconds}s, max_messages={max_messages}"
        )
...
class FileCacheService:
    ...
    async def set_file_metadata(
        file_id: str, metadata: Dict, expire_seconds: int = 3600
    ):
    ...
    async def set_upload_progress(
        upload_id: str, progress: Dict, expire_seconds: int = 1800
    ):
...
class AnalyticsCacheService:
    ...
    @staticmethod
    async def cache_analytics_report(
        report_name: str, data: Dict, expire_seconds: int = 3600
    ):
...
class CacheUtils:
    ...
    async def invalidate_related_cache(
        user_id: str = None, group_id: str = None, chat_id: str = None
    ):
        ...
        if group_id:
            patterns.extend([f"group:members:{group_id}", f"group:info:{group_id}"])

        if chat_id:
            patterns.extend([f"chat_messages:{chat_id}"])
...
async def init_cache():
    ...
    if os.getenv("USE_MOCK_CACHE", "false").lower() in ("true", "1", "yes"):
        logger.info("[REDIS] USE_MOCK_CACHE enabled - using in-memory cache only")
        await cache.clear_mock_cache()
        return True

    # Check for DEBUG mode which allows fallback
    debug_mode = os.getenv("DEBUG", "false").lower() in ("true", "1", "yes")

    # Check if running locally (not in Docker) - allow fallback to mock cache
    redis_host_candidate = getattr(settings, "REDIS_HOST", "redis")
    if redis_host_candidate == "redis" and not os.path.exists("/.dockerenv"):
        # Likely running locally without Docker - use mock cache
        logger.warning(
            "[REDIS] Local development detected - using in-memory mock cache"
        )
        logger.warning(
            "[REDIS] For production with Redis, set USE_MOCK_CACHE=false or run in Docker"
        )
        await cache.clear_mock_cache()
        return True
    ...
    if redis_url:
        ...
        try:
            ...
            logger.error(
                f"[REDIS] Failed to parse REDIS_URL, falling back to components: {e}"
            )
        ...
        redis_db = getattr(settings, "REDIS_DB", 0)
        logger.info(
            f"[REDIS] Using configured components: {redis_host}:{redis_port}/{redis_db}"
        )

    # CRITICAL: Enforce docker service name in production
    if redis_host in ("localhost", "127.0.0.1", "::1"):
        logger.error(
            f"[REDIS] CRITICAL: Redis host is localhost - must use docker service name 'redis'"
        )
        redis_host = "redis"
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"[REDIS] Connection attempt {attempt + 1}/{max_retries} to {redis_host}:{redis_port} db={redis_db}"
            )
            ...
            try:
                ...
                logger.error(
                    f"[REDIS] Connection attempt {attempt + 1} failed: {type(connect_e).__name__}: {connect_e}"
                )
            ...
            if connected:
                logger.info(
                    f"[REDIS] Successfully connected to {redis_host}:{redis_port}/{redis_db}"
                )
                ...
                try:
                    ...
                    if test_result == "test":
                        logger.info(
                            f"[REDIS] Redis functionality verified - cache ready"
                        )
                        return True
                    else:
                        logger.error(
                            f"[REDIS] Redis test failed - expected 'test', got: {test_result}"
                        )
                        raise RuntimeError(
                            f"Redis functionality test returned unexpected value: {test_result}"
                        )
                except Exception as test_e:
                    logger.error(
                        f"[REDIS] Functionality test failed: {type(test_e).__name__}: {test_e}"
                    )
            ...
            logger.error(
                f"[REDIS] Connection timeout attempt {attempt + 1}/{max_retries}: {e}"
            )
            logger.error(
                f"[REDIS] Timeout typically means: Redis not responding, network latency, or connectivity issue"
            )
            last_error = e
            if attempt < max_retries - 1:
                delay = base_delay * (2**attempt)  # Exponential backoff
                logger.warning(
                    f"[REDIS] Retrying in {delay} seconds... (attempt {attempt + 1}/{max_retries})"
                )
                await asyncio.sleep(delay)
            else:
                logger.warning(
                    f"[REDIS] All {max_retries} attempts exhausted - falling back to in-memory mock cache"
                )
                await cache.clear_mock_cache()
                return True
            ...
            logger.error(
                f"[REDIS] Connection error attempt {attempt + 1}/{max_retries}: {type(e).__name__}: {e}"
            )
            import traceback
            ...
            if attempt < max_retries - 1:
                ...
                if "authentication" in str(e).lower() or "WRONGPASS" in str(e):
                    logger.error(
                        f"[REDIS] AUTHENTICATION FAILURE: Check REDIS_PASSWORD environment variable"
                    )
                await cache.clear_mock_cache()
                return True
...
class MessageQueueService:
    ...
    async def enqueue_message(message_data: dict, ttl_minutes: int = 60):
        ...
        await cache.publish(
            f"chat:{message_data.get('chat_id')}",
            {
                "type": "new_message",
    ...
    @staticmethod
    async def fanout_message_to_devices(
        message_data: dict, recipient_devices: List[str], ttl_minutes: int = 60
    ):
        """
        ...
        for device_id in recipient_devices:
            ...
            await cache.set(
                delivery_key, delivery_state, expire_seconds=ttl_minutes * 60
            )
    ...
    async def update_message_state(message_id: str, device_id: str, new_state: str):
        ...
        await cache.set(
            delivery_key, current_state, expire_seconds=60 * 60 * 24
        )  # 24h TTL

        # Publish state update
        await cache.publish(
            f"message_state:{message_id}",
            {
                "message_id": message_id,
                "device_id": device_id,
                "state": new_state,
    ...
    @staticmethod
    async def get_pending_messages_for_device(
        device_id: str, chat_id: str, limit: int = 50
    ):
        """
    ...
    async def add_retry_attempt(message_id: str, device_id: str):
        ...
        if retry_data["count"] <= len(backoff_intervals):
            retry_data["next_retry"] = (
                ...
                + timedelta(seconds=backoff_intervals[retry_data["count"] - 1])
            ).isoformat()
...
class DeviceTrustGraphService:
    ...
    async def register_primary_device(user_id: str, device_id: str, device_info: dict):
        ...
        await cache.set(
            trust_key, trust_graph, expire_seconds=30 * 24 * 60 * 60
        )  # 30 days TTL
    ...
    async def revoke_device(
        user_id: str, primary_device_id: str, device_to_revoke: str, signature: str
    ):
        """
    ...
    async def update_device_heartbeat(user_id: str, device_id: str):
        ...
        if trust_graph and device_id in trust_graph["linked_devices"]:
            trust_graph["linked_devices"][device_id]["last_seen"] = datetime.now(
                timezone.utc
            ).isoformat()
            trust_graph["last_updated"] = datetime.now(timezone.utc).isoformat()
    ...
    @staticmethod
    async def promote_linked_device(
        user_id: str, primary_device_id: str, device_id: str, signature: str
    ):
        """
        ...
        # Promote device
        trust_graph["linked_devices"][device_id]["permissions"] = [
            "read",
            "send",
            "admin",
        ]
        trust_graph["linked_devices"][device_id]["promoted_by"] = primary_device_id
        trust_graph["linked_devices"][device_id]["promoted_at"] = datetime.now(
            timezone.utc
        ).isoformat()
        trust_graph["last_updated"] = datetime.now(timezone.utc).isoformat()
...
class GroupSenderKeyService:
    ...
    async def create_group_sender_key(
        ...
        await cache.set(
            creator_key,
            {**sender_key_data, "metadata": sender_key_metadata},
        ...
        await cache.hset(
            registry_key, {f"{creator_user_id}:{creator_device_id}": sender_key_id}
        )
        ...
        for member_id in initial_members:
            if member_id != creator_user_id:
                await cache.publish(
                    f"group_key_distribution:{group_id}",
                    {
                        "type": "new_sender_key",
                        "group_id": group_id,
                        "sender_key_id": sender_key_id,
    ...
    @staticmethod
    async def distribute_sender_key_to_member(
        group_id: str,
        sender_key_id: str,
        target_user_id: str,
        ...
        await cache.hset(
            registry_key, {f"{target_user_id}:{target_device_id}": sender_key_id}
        )
    ...
    async def get_group_sender_keys(group_id: str, user_id: str, device_id: str):
        ...
        for user_device, sender_key_id in all_registrations.items():
            ...
            if sender_key_data:
                ...
                if (
                    user_device == f"{user_id}:{device_id}"
                    or await GroupSenderKeyService._should_have_key(
                        group_id, user_id, device_id, user_device
                    )
                ):
    ...
    @staticmethod
    async def _should_have_key(
        group_id: str,
        requesting_user_id: str,
        requesting_device_id: str,
        key_owner_user_device: str,
    ):
    ...
    @staticmethod
    async def rotate_group_sender_keys(
        group_id: str, trigger_user_id: str, trigger_device_id: str, reason: str
    ):
        """
    ...
    @staticmethod
    async def get_group_state_history(
        group_id: str, from_sequence: int = 0, limit: int = 50
    ):
        ...
        for seq in range(
            from_sequence + 1, min(current_seq + 1, from_sequence + limit + 1)
        ):
...
class PushNotificationService:
    ...
    @staticmethod
    async def create_encrypted_push_notification(
        message_data: dict, recipient_devices: List[str]
    ):
        """
    ...
    async def queue_push_notifications(encrypted_notifications: List[dict]):
        ...
        for notification in encrypted_notifications:
            ...
            await cache.publish(
                f"push_notifications:{notification['device_id']}",
                {
                    "type": "new_push",
    ...
    @staticmethod
    async def create_typing_push_notification(
        chat_id: str, typing_user_id: str, recipient_devices: List[str]
    ):
        """
    ...
    @staticmethod
    async def create_delivery_receipt_push_notification(
        message_id: str, chat_id: str, receipt_data: dict, recipient_devices: List[str]
    ):
        """
...
class MetadataMinimizationService:
    ...
    @staticmethod
    def create_minimal_message_metadata(
        message_id: str,
        chat_id: str,
        sender_user_id: str,
    ...
    @staticmethod
    def create_minimal_chat_metadata(
        chat_id: str, creator_user_id: str, chat_type: str, member_count: int
    ):
        """
        ...
        if document_type in forbidden_fields:
            for field in forbidden_fields[document_type]:
                if field in document:
                    raise ValueError(
                        f"Forbidden field '{field}' found in {document_type} metadata"
                    )
    ...
    async def enforce_metadata_limits():
        ...
        if (
            not storage_stats.get("error")
            and storage_stats.get("estimated_storage_mb", 0) > 1000
        ):  # 1GB limit
            try:
                try:
                    from .db_proxy import get_database
                ...
                messages_collection = db.messages
                oldest_messages = (
                    messages_collection.find().sort("created_at", 1).limit(1000)
                )
                ...
                stats[
                    "additional_cleanup"
                ] = f"Removed {len(old_message_ids)} oldest messages"
...
class MetadataCollectionService:
    ...
    async def collect_message_metadata(
        self,
        message_id: str,
        sender_id: str,
        recipient_id: str,
    ...
    async def collect_user_presence_metadata(
        self, user_id: str, device_id: str, status: str, client_ip: str
    ) -> Dict[str, Any]:
        ...
        await self.cache.set(
            presence_key, presence_metadata, expire_seconds=24 * 60 * 60
        )
    ...
    async def collect_delivery_metadata(
        self, message_id: str, recipient_id: str, device_id: str, delivery_type: str
    ) -> Dict[str, Any]:
        ...
        await self.cache.set(
            delivery_key, delivery_metadata, expire_seconds=7 * 24 * 60 * 60
        )

        return delivery_metadata

    async def get_user_metadata_summary(
    ...
    async def _update_device_activity(self, device_id: str, user_id: str):
        ...
        await self.cache.hset(
            activity_key, "last_activity", activity_data["last_activity"]
        )
...
class RelationshipGraphService:
    ...
    async def update_relationship_strength(
        self, user_a: str, user_b: str, interaction_type: str, weight: float = 1.0
    ):
    ...
    async def get_user_relationships(
        ...
        relationships.sort(
            key=lambda x: x.get("relationship_strength", 0), reverse=True
        )
    ...
    async def get_contact_suggestions(
        ...
        for rel in relationships:
            other_user = (
                rel.get("user_b") if rel.get("user_a") == user_id else rel.get("user_a")
            )
            if other_user and other_user != user_id:
                # Get relationships of this user
                other_relationships = await self.get_user_relationships(
                    other_user, limit=20
                )
                for other_rel in other_relationships:
                    suggested_user = (
                        other_rel.get("user_b")
                        if other_rel.get("user_a") == other_user
                        else other_rel.get("user_a")
                    )
                    if (
                        suggested_user
                        and suggested_user != user_id
                        and suggested_user not in suggestions
                    ):
                        # Calculate suggestion strength
                        strength = rel.get("relationship_strength", 0) * other_rel.get(
        ...
        sorted_suggestions = sorted(
            suggestions.values(), key=lambda x: x["suggestion_strength"], reverse=True
        )

        return sorted_suggestions[:limit]

    async def calculate_interaction_frequency(
        self, user_a: str, user_b: str, days: int = 30
    ) -> float:
...
class DeviceTrackingService:
    ...
    async def update_device_activity(
        self, user_id: str, device_id: str, activity_type: str
    ):
        ...
        activity_key = f"device_activity:{device_id}"
        await self.cache.hset(
            activity_key, "last_activity", datetime.now(timezone.utc).isoformat()
        )
...
class MessageMetadataService:
    ...
    async def update_message_status(self, message_id: str, status: str, device_id: str):
        ...
        await self.cache.hset(
            metadata_key,
            f"status_{device_id}_timestamp",
            datetime.now(timezone.utc).isoformat(),
        )
        await self.cache.expire(metadata_key, self.metadata_ttl)

    async def get_chat_metadata_summary(
        self, chat_id: str, days: int = 30
    ) -> Dict[str, Any]:
