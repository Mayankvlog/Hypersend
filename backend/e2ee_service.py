...
try:
    from backend.crypto.signal_protocol import (
        SignalProtocol,
        X3DHBundle,
        IdentityKeyPair,
        X3DHHandshake,
    )
    from backend.crypto.multi_device import (
        MultiDeviceManager,
        DeviceInfo,
        DeviceSession,
    )
    ...
    from crypto.signal_protocol import (
        SignalProtocol,
        X3DHBundle,
        IdentityKeyPair,
        X3DHHandshake,
...
class E2EEService:
    ...
    def __init__(self, db=None, redis_client=None):
        ...
        self.abuse_detection = (
            AbuseDetectionService(redis_client) if redis_client else None
        )
    ...
    async def initiate_session_with_x3dh(
        ...
        try:
            logger.info(
                f"🔐 X3DH initiation: {initiator_device_id} → {recipient_device_id}"
            )

            # 1. Fetch recipient's public key bundle
            if not self.device_manager:
                logger.error("device_manager not available")
                raise E2EECryptoError("Device manager not available")

            recipient_bundle = await self.device_manager.get_user_key_bundle(
                user_id=recipient_user_id, device_id=recipient_device_id
            )
            ...
            x3dh_result = X3DHKeyExchange.initiate_session(
                ...
                receiver_signed_prekey_signature_b64=recipient_bundle[
                    "signed_prekey_signature"
                ],
                receiver_one_time_prekey_public_b64=recipient_bundle.get(
                    "one_time_prekey"
                ),
                receiver_identity_key_for_verification_b64=recipient_bundle[
                    "identity_key"
                ],
            )
            ...
            double_ratchet = DoubleRatchet(
                shared_secret_b64, dh_send_pair=initiator_ephemeral_pair
            )
            sending_chain_b64 = double_ratchet.create_sending_chain_key()

            # 4. Create session state
            session_id = (
                f"{initiator_device_id}_{recipient_device_id}_{secrets.token_hex(16)}"
            )
    ...
    async def receive_session_from_x3dh(
        ...
        try:
            logger.info(
                f"🔐 X3DH reception: {initiator_device_id} → {receiver_device_id}"
            )
            ...
            receiving_chain_b64 = double_ratchet.create_receiving_chain_key(
                initiator_ephemeral_public_b64
            )

            # 3. Create session state
            session_id = (
                f"{initiator_device_id}_{receiver_device_id}_{secrets.token_hex(16)}"
            )
            session_state = DeviceSessionState(
    ...
    async def encrypt_and_send_message(
        ...
        try:
            ...
            (
                ...
                new_chain_key_b64,
                counter,
            ) = double_ratchet.ratchet_sending_chain()
    ...
    async def receive_and_decrypt_message(
        ...
        try:
            ...
            (
                ...
                new_chain_key_b64,
                recv_counter,
            ) = double_ratchet.ratchet_receiving_chain()
    ...
    async def queue_message_for_retry(
        ...
        try:
            ...
            retry_queue_key = (
                f"{self.MESSAGE_RETRY_PREFIX}:{sender_user_id}:{sender_device_id}"
            )
            ...
            return {
                ...
                "retry_count": 0,
                "next_retry_at": (
                    timestamp + timedelta(seconds=self.RETRY_BACKOFF_SCHEDULE[0])
                ).isoformat(),
            }
    ...
    async def retry_pending_messages(
        self, sender_user_id: str, sender_device_id: str
    ) -> Dict:
        """
        ...
        try:
            ...
            for retry_entry_json in retry_queue:
                ...
                last_attempt = (
                    datetime.fromisoformat(retry_entry["last_attempt_at"])
                    if retry_entry["last_attempt_at"]
                    else now
                )

                # Check if backoff period has elapsed
                backoff_seconds = self.RETRY_BACKOFF_SCHEDULE[
                    min(retry_count, len(self.RETRY_BACKOFF_SCHEDULE) - 1)
                ]
                ...
                if now < next_retry_time:
                    ...
                    logger.debug(
                        f"⏳ Message {message_id} retry scheduled for {next_retry_time.isoformat()}"
                    )
                ...
                if retry_count >= self.MAX_RETRY_ATTEMPTS:
                    ...
                    logger.warning(
                        f"❌ Message {message_id} exceeded max retries ({self.MAX_RETRY_ATTEMPTS})"
                    )
                    state_key = f"{self.MESSAGE_STATE_PREFIX}:{message_id}"
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Message {message_id} retried successfully (attempt {retry_count + 1})"
                    )
                    ...
                    logger.warning(
                        f"⚠️ Message {message_id} retry attempt {retry_count + 1} failed: {send_err}"
                    )
                    retry_entry["retry_count"] += 1
                    retry_entry["last_attempt_at"] = now.isoformat()

                    next_idx = min(
                        retry_entry["retry_count"], len(self.RETRY_BACKOFF_SCHEDULE) - 1
                    )
                    retry_entry["next_retry_in_seconds"] = self.RETRY_BACKOFF_SCHEDULE[
                        next_idx
                    ]
            ...
            logger.info(
                f"📊 Retry processing complete: {results['total_processed']} messages processed"
            )
            return results
    ...
    async def track_delivery_receipt(
        ...
        try:
            ...
            if receipt_type == "read":
                ...
                logger.info(
                    f"✓ Message {message_id} marked as read by {recipient_device_id}"
                )
                ...
                logger.info(
                    f"✓ Message {message_id} delivered to {recipient_device_id}"
                )
    ...
    async def mark_device_online(self, user_id: str, device_id: str) -> Dict:
        """
    ...
    async def initialize_user_protocol(
        self, user_id: str, device_id: str = "primary"
    ) -> str:
        """
    ...
    async def encrypt_media_file(
        ...
        try:
            ...
            encrypted_media = await self.media_encryption.encrypt_media(
                file_data, file_type, view_once=view_once, ttl_seconds=ttl_seconds
            )
    ...
    async def create_group_encryption(
        self, group_id: str, creator_user_id: str, member_user_ids: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for member_user_id in member_user_ids:
                ...
                key_distribution = creator_protocol.add_group_member(
                    group_id, member_device_id
                )
            ...
            logger.info(
                f"Group encryption created: {group_id} ({len(member_user_ids)} members)"
            )
    ...
    async def send_group_message(
        self, group_id: str, sender_user_id: str, message_text: str
    ) -> Dict[str, Any]:
        """
    ...
    async def check_message_velocity(
        self, user_id: str, message_count: int = 1
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            velocity_result = await self.abuse_detection.check_message_velocity(
    ...
    async def _store_identity_keys(
        self, user_id: str, device_id: str, protocol: SignalProtocol
    ):
        ...
        key_data = {
            ...
            "signed_pre_key": base64.b64encode(
                protocol.signed_pre_key.key_pair
            ).decode(),
            "one_time_pre_keys": [
                {
                    "key_id": key.key_id,
    ...
    async def _store_encrypted_media(
        self, media_id: str, encrypted_media: Dict[str, Any]
    ):
    ...
    async def _queue_group_message(self, group_message: Dict[str, Any]):
        ...
        await self.redis.lpush(
            f"group_queue:{group_message['group_id']}", json.dumps(group_message)
        )
    ...
    async def mark_device_offline(self, user_id: str, device_id: str) -> Dict:
        """
...
class AbuseAndSpamScoringService:
    ...
    async def increment_abuse_score(
        self, user_id: str, violation_type: str, increment: float, reason: str = None
    ) -> Dict:
        """
        ...
        try:
            ...
            if new_score >= self.SUSPENSION_THRESHOLD:
                action = "suspended"
                action_expires_at = (
                    timestamp + timedelta(hours=self.SUSPENSION_DURATION_HOURS)
                ).isoformat()
                ...
                action_expires_at = (
                    timestamp + timedelta(hours=self.SHADOW_BAN_DURATION_HOURS)
                ).isoformat()
            ...
            logger.info(
                f"⚠️  Abuse score updated: {user_id} | old:{old_score:.2f} → new:{new_score:.2f} | type:{violation_type}"
            )
            ...
            if self.MODERATION_ENABLED and new_score >= self.AUTO_REPORT_THRESHOLD:
                await self._add_to_moderation_queue(
                    user_id, new_score, violation_type, reason
                )
    ...
    async def check_message_velocity(
        ...
        try:
            ...
            if minute_count > self.MESSAGE_VELOCITY_LIMIT_PER_MINUTE:
                ...
                logger.warning(
                    f"⚠️  User {user_id} exceeded minute velocity: {minute_count} > {self.MESSAGE_VELOCITY_LIMIT_PER_MINUTE}"
                )
            ...
            if hour_count > self.MESSAGE_VELOCITY_LIMIT_PER_HOUR:
                ...
                logger.warning(
                    f"⚠️  User {user_id} exceeded hour velocity: {hour_count} > {self.MESSAGE_VELOCITY_LIMIT_PER_HOUR}"
                )
    ...
    async def track_unique_recipients(
        self, user_id: str, recipient_user_ids: List[str]
    ) -> Dict:
        """
        ...
        try:
            ...
            if hour_unique_count > self.UNIQUE_RECIPIENTS_HOURLY_LIMIT:
                ...
                logger.warning(
                    f"⚠️  User {user_id} exceeded hourly recipient limit: {hour_unique_count} > {self.UNIQUE_RECIPIENTS_HOURLY_LIMIT}"
                )
            ...
            if day_unique_count > self.UNIQUE_RECIPIENTS_DAILY_LIMIT:
                ...
                logger.warning(
                    f"⚠️  User {user_id} exceeded daily recipient limit: {day_unique_count} > {self.UNIQUE_RECIPIENTS_DAILY_LIMIT}"
                )
    ...
    async def process_abuse_report(
        ...
        try:
            ...
            logger.info(
                f"📋 Abuse report created: {report_id} | reported:{reported_user_id} | type:{report_type}"
            )
    ...
    async def _add_to_moderation_queue(
        self, user_id: str, score: float, violation_type: str, reason: str
    ) -> None:
        """
        ...
        try:
            ...
            logger.info(
                f"📬 User added to moderation queue: {user_id} (score={score:.2f})"
            )

        except Exception as e:
            logger.error(f"Failed to add to moderation queue: {e}")

    async def apply_enforcement_action(self, user_id: str, action: str) -> Dict:
        """
    ...
    async def decay_abuse_score(self, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"📉 Abuse score decayed: {user_id} | old:{current_score:.2f} → new:{new_score:.2f}"
            )

            # If score drops below shadow ban threshold, lift shadow ban
            if (
                current_score >= self.SHADOW_BAN_THRESHOLD
                and new_score < self.SHADOW_BAN_THRESHOLD
            ):
...
class GroupEncryptionService:
    ...
    async def create_group_sender_key(
        self, group_id: str, sender_user_id: str, sender_device_id: str
    ) -> Dict:
        """
        ...
        try:
            ...
            logger.info(
                f"🔐 Group sender key created: {group_id} | sender:{sender_device_id}"
            )
    ...
    async def get_group_message_sequence(self, group_id: str) -> int:
        """
    ...
    async def fanout_group_message(
        ...
        try:
            ...
            logger.info(
                f"📢 Group message fanned out: {message_id} → {len(fanout_status['recipient_devices'])} members"
            )
...
class PresenceAndTypingService:
    ...
    async def broadcast_typing(
        self, chat_id: str, user_id: str, device_id: str, is_typing: bool = True
    ) -> Dict:
        """
...
class BackgroundWorkerCoordinator:
    ...
    async def enqueue_retry_job(
        self, message_id: str, sender_user_id: str, retry_count: int = 0
    ) -> str:
        """
    ...
    async def execute_retry_job(self, job_id: str) -> Dict:
        ...
        try:
            ...
            if delivery_ok:
                ...
                await self.redis.set(
                    f"job:{new_job['job_id']}", json.dumps(new_job), ex=86400
                )
                await self.redis.lpush("queue:retry", new_job["job_id"])

                logger.info(
                    f"Retry requeued: {message_id}:{device_id} (attempt {next_attempt})"
                )
...
async def get_typing_cleanup_executor(
    db=None, redis_client=None
) -> TypingCleanupExecutor:
    """Factory for typing cleanup executor."""
    return TypingCleanupExecutor(db=db, redis_client=redis_client)


async def get_group_key_distribution_executor(
    db=None, redis_client=None
) -> GroupKeyDistributionExecutor:
...
async def get_presence_and_typing_service(
    db=None, redis_client=None
) -> PresenceAndTypingService:
    """Factory for presence service."""
    return PresenceAndTypingService(db=db, redis_client=redis_client)


async def get_background_worker_coordinator(
    db=None, redis_client=None
