...
def cleanup_expired_lockouts():
    ...
    expired_keys = [
        key
        for key, expiry_time in persistent_login_lockouts.items()
...
def _is_valid_origin_format(origin: str) -> bool:
    ...
    try:
        ...
        port = (
            parsed.group(3) if len(parsed.groups()) >= 3 and parsed.group(3) else None
        )
...
async def authenticate_device(device_request: dict, request: Request) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            redis_client.setex(
                device_session_key, 86400, str(auth_data)
            )  # 24 hours TTL
...
async def register(user: UserCreate) -> UserResponse:
    ...
    try:
        ...
        try:
            ...
            auth_log(
                f"Database connection error checking existing user: {type(db_error).__name__}: {str(db_error)}"
            )
            raise HTTPException(
            ...
            auth_log(
                f"Database error checking existing user: {type(db_error).__name__}: {str(db_error)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        if existing_user:
            user_id = (
                existing_user.get("_id") if isinstance(existing_user, dict) else None
            )
            auth_log(
                f"Registration failed: Found existing user with email: {normalized_email} (ID: {user_id})"
            )
        else:
            auth_log(
                f"Registration: No existing user found for email: {normalized_email}"
            )
        ...
        try:
            ...
            if hasattr(inserted_id, "__await__") or asyncio.isfuture(inserted_id):
                raise RuntimeError(
                    f"Critical async error: inserted_id is a Future object: {type(inserted_id)}"
                )

            # Ensure database session is committed by verifying insert
            verification_user = await _await_maybe(
                users_col.find_one({"_id": inserted_id}), timeout=5.0
            )
            if not verification_user:
                raise RuntimeError(
                    "Failed to verify user insertion - database session may not have committed"
                )

            # Convert ObjectId to string for JWT token
            user_id_str = str(inserted_id)
            auth_log(
                f"SUCCESS: User registered successfully: {user.email} (Atlas ObjectId: {user_id_str})"
            )
            ...
            auth_log(
                f"Database connection error during user insertion: {type(db_error).__name__}: {str(db_error)}"
            )
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Database service temporarily unavailable. Please try again.",
            ...
            auth_log(
                f"Database error during user insertion: {type(db_error).__name__}: {str(db_error)}"
            )
    ...
    try:
        ...
        if client_ip in persistent_login_lockouts:
            ...
            if current_time < lockout_expiry:
                ...
                auth_log(
                    f"IP {client_ip} is locked out for {remaining_seconds} more seconds"
                )
                raise HTTPException(
        ...
        if email_lockout_key in persistent_login_lockouts:
            ...
            if current_time < lockout_expiry:
                remaining_seconds = int((lockout_expiry - current_time).total_seconds())
                auth_log(
                    f"Email {credentials.email} is locked out for {remaining_seconds} more seconds"
                )
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
        ...
        if len(login_attempts[client_ip]) >= MAX_LOGIN_ATTEMPTS_PER_IP:
            auth_log(
                f"IP {client_ip} exceeded max login attempts ({MAX_LOGIN_ATTEMPTS_PER_IP})"
            )
        ...
        try:
            ...
            try:
                ...
                auth_log(
                    f"Database connection error during user lookup: {type(db_error).__name__}: {str(db_error)}"
                )
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                ...
                auth_log(
                    f"Database error during user lookup: {type(db_error).__name__}: {str(db_error)}"
                )
                if (
                    "connection" in str(db_error).lower()
                    or "network" in str(db_error).lower()
                ):
                    raise HTTPException(
                        status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                        detail="Database service temporarily unavailable",
            ...
            if not password_hash:
                auth_log(
                    f"Login failed: User {normalized_email} has no password hash (corrupted record)"
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Invalid email/username or password",
            ...
            if not password_salt:
                auth_log(
                    f"DEBUG: User {normalized_email} has no password salt, attempting migration"
                )
                ...
                if (
                    ...
                    if len(parts) == 2:
                        ...
                        auth_log(
                            f"Migrating legacy password format for {normalized_email}"
                        )
                        ...
                        try:
                            ...
                            auth_log(
                                f"Successfully migrated password format for {normalized_email}"
                            )
                        except Exception as migrate_error:
                            auth_log(
                                f"Failed to migrate password for {normalized_email}: {migrate_error}"
                            )
                            # Continue with legacy format for now
                    else:
                        auth_log(
                            f"Invalid legacy password format for {normalized_email}"
                        )
                else:
                    ...
                    if (
                        ...
                        if len(password_hash) == 97:
                            ...
                            if len(parts) == 2:
                                password_salt, password_hash = parts
                                auth_log(
                                    f"Found combined password format for {normalized_email}"
                                )
                                ...
                                try:
                                    ...
                                    auth_log(
                                        f"Successfully separated password format for {normalized_email}"
                                    )
                                except Exception as migrate_error:
                                    auth_log(
                                        f"Failed to separate password for {normalized_email}: {migrate_error}"
                                    )
                            else:
                                auth_log(
                                    f"Invalid combined password format for {normalized_email}"
                                )
                        else:
                            auth_log(
                                f"Invalid combined password length for {normalized_email}: {len(password_hash)}"
                            )

                    # If still no password salt after migration attempt
                    if not password_salt:
                        auth_log(
                            f"Login failed: User {normalized_email} has no valid password salt (corrupted record)"
                        )
                        raise HTTPException(
                            status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Invalid email/username or password",
            ...
            if settings.DEBUG:
                auth_log(
                    f"[PASSWORD_DEBUG] Attempting password verification for {normalized_email} (hash present: {bool(password_hash)}, salt present: {bool(password_salt)})"
                )

            if (
                password_salt
                and isinstance(password_salt, str)
                and len(password_salt) > 0
            ):
                ...
                if settings.DEBUG:
                    ...
                    if password_salt:
                        ...
                        pwd_salt_hash = hashlib.sha256(
                            (credentials.password + password_salt).encode()
                        ).hexdigest()
                        salt_pwd_hash = hashlib.sha256(
                            (password_salt + credentials.password).encode()
                        ).hexdigest()
                        auth_log(
                            f"[DEBUG] Testing legacy password formats (hashes not logged for security)"
                        )
                        auth_log(
                            f"[DEBUG] Pwd+Salt match: {pwd_salt_hash == password_hash}"
                        )
                        auth_log(
                            f"[DEBUG] Salt+Pwd match: {salt_pwd_hash == password_hash}"
                        )

                        # Try alternative
                        combined_test_alt = password_salt + credentials.password
                        expected_hash_alt = hashlib.sha256(
                            combined_test_alt.encode()
                        ).hexdigest()
                        auth_log(
                            f"[DEBUG] Alternative matches: {expected_hash_alt == password_hash}"
                        )

                # CRITICAL FIX: If verification still fails, provide helpful error for password reset
                if not is_password_valid:
                    auth_log(
                        f"[PASSWORD_RESET_NEEDED] All verification methods failed for {normalized_email}"
                    )
                    auth_log(
                        f"[PASSWORD_RESET_NEEDED] User should reset via /auth/reset-password with a token"
                    )
                ...
                if (
                    ...
                    and password_salt
                    and isinstance(password_salt, str)
                    and len(password_salt) == 64
                ):
                    auth_log(
                        f"[RECOVERY] Trying swapped hash/salt for {normalized_email}"
                    )
                    is_password_valid = verify_password(
                        credentials.password,
                        password_salt,
                    ...
                    if is_password_valid:
                        auth_log(
                            f"[RECOVERY] SUCCESS: Hash and salt were swapped for {normalized_email}, fixing in database"
                        )
                        # Fix the database
                        try:
                            await users_collection().update_one(
                            ...
                            auth_log(
                                f"[RECOVERY] Database corrected for {normalized_email}"
                            )
                ...
                if "$" in password_hash and len(password_hash) == 97:
                    ...
                    auth_log(
                        f"Using legacy combined password format for {normalized_email}"
                    )
                    is_password_valid = verify_password(
                        credentials.password,
                        password_hash,
                ...
                auth_log(
                    f"Unrecognized password format for {normalized_email}: hash_len={len(password_hash) if password_hash else 0}, hash_has_$={('$' in password_hash) if password_hash else False}, salt_len={len(password_salt) if password_salt else 0}"
                )
                is_password_valid = False

            auth_log(
                f"Password verification result for {normalized_email}: {is_password_valid} (hash_length: {len(password_hash) if password_hash else 0}, has_salt: {bool(password_salt)})"
            )
        except HTTPException:
            raise
        except Exception as verify_error:
            auth_log(
                f"Password verification error for {normalized_email}: {type(verify_error).__name__}"
            )
        ...
        if not is_password_valid:
            ...
            auth_log(
                f"[DIAGNOSIS] Hash format: {diagnosis['hash']['format']} (len={diagnosis['hash']['length']}, hex={diagnosis['hash']['is_hex']})"
            )
            if diagnosis["hash"]["details"]:
                auth_log(f"[DIAGNOSIS] Hash details: {diagnosis['hash']['details']}")
            auth_log(
                f"[DIAGNOSIS] Salt format: {diagnosis['salt']['format']} (len={diagnosis['salt']['length']}, hex={diagnosis['salt']['is_hex']})"
            )
            auth_log(
                f"[DIAGNOSIS] Combined format detected: {diagnosis['combined_format']}"
            )
            ...
            if attempt_count >= MAX_FAILED_ATTEMPTS_PER_ACCOUNT:
                lockout_seconds = PROGRESSIVE_LOCKOUTS.get(
                    min(attempt_count, 5), 1800
                )  # Max 30 min
                lockout_time = current_time + timedelta(seconds=lockout_seconds)
                persistent_login_lockouts[email_lockout_key] = lockout_time
                auth_log(
                    f"Account {normalized_email} locked out for {lockout_seconds} seconds after {attempt_count} failed attempts"
                )
        ...
        cookie_domain = (
            ".zaply.in.net"  # CRITICAL: Use leading dot for subdomain compatibility
        )
...
async def refresh_session_token(request: Request) -> JSONResponse:
    ...
    try:
        ...
        try:
            ...
            if isinstance(user_id_for_query, str) and ObjectId.is_valid(
                user_id_for_query
            ):
            ...
            auth_log(
                f"Database error checking refresh token: {type(e).__name__}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        try:
            user = await asyncio.wait_for(
                users_collection().find_one({"_id": user_id_for_query}), timeout=5.0
            )
        except asyncio.TimeoutError:
            raise HTTPException(
                status_code=status.HTTP_504_GATEWAY_TIMEOUT,
        ...
        if "created_at" not in refresh_doc:
            auth_log(
                f"SECURITY WARNING: Refresh token missing created_at field - rejecting refresh for user: {token_data.user_id}, token_jti: {token_data.jti}"
            )
            # Invalidate the token with missing created_at
            await refresh_tokens_collection().update_one(
                {"_id": refresh_doc["_id"]}, {"$set": {"invalidated": True}}
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        if token_created_at.tzinfo is None:
            ...
            auth_log(
                f"SECURITY INFO: Normalized naive created_at to UTC for user: {token_data.user_id}, token_jti: {token_data.jti}, original: {refresh_doc['created_at']}, normalized: {token_created_at}"
            )
        elif (
            token_created_at.tzinfo is not None
            and token_created_at.utcoffset() != timedelta(0)
        ):
            # Convert to UTC if it's in a different timezone
            token_created_at = token_created_at.astimezone(timezone.utc)
            auth_log(
                f"SECURITY INFO: Converted created_at to UTC for user: {token_data.user_id}, token_jti: {token_data.jti}, original: {refresh_doc['created_at']}, converted: {token_created_at}"
            )

        token_max_expiry = token_created_at + timedelta(
            days=REFRESH_TOKEN_MAX_LIFETIME_DAYS
        )
        current_time = datetime.now(timezone.utc)
        ...
        if current_time >= token_max_expiry:
            ...
            auth_log(
                f"Token refresh rejected - absolute max lifetime exceeded for user: {token_data.user_id}, token_age: {token_age_days} days, max_lifetime: {REFRESH_TOKEN_MAX_LIFETIME_DAYS} days, token_jti: {token_data.jti}"
            )
        ...
        access_expires_timestamp = int(
            (datetime.now(timezone.utc) + access_token_expires).timestamp()
        )
...
async def refresh_access_token(request: RefreshTokenRequest) -> Token:
    ...
    try:
        ...
        auth_log(
            f"SUCCESS: Token refreshed successfully for user: {token_data.user_id}"
        )
...
async def reset_password(request: PasswordResetRequest) -> PasswordResetResponse:
    ...
    try:
        ...
        # FIXED: Password reset functionality enabled
        auth_log(
            f"Password reset functionality enabled: {settings.ENABLE_PASSWORD_RESET}"
        )
        ...
        try:
            ...
            auth_log(
                f"Database error checking reset token hash: {type(e).__name__}: {str(e)}"
            )
            user = None
        ...
        auth_log(
            f"Password reset validated using Atlas hashed token for user: {user['_id']}"
        )
        ...
        try:
            ...
            auth_log(
                f"Warning: Failed to clear reset token fields: {type(e).__name__}: {str(e)[:120]}"
            )

        # Invalidate all refresh tokens for this user - forces re-login
        try:
            auth_log(
                f"[RESET_PASSWORD] Invalidating all refresh tokens for user: {user['_id']}"
            )
            await refresh_tokens_collection().update_many(
                {"user_id": user["_id"]},
                {
            ...
            auth_log(
                f"Warning: Failed to invalidate refresh tokens: {type(e).__name__}"
            )
...
async def change_password(
    request: ChangePasswordRequest, current_user: str = Depends(get_current_user)
) -> PasswordResetResponse:
    ...
    try:
        ...
        if not old_password:
            auth_log(
                f"[CHANGE_PASSWORD_ERROR] No password field provided for user {current_user}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            auth_log(
                f"[CHANGE_PASSWORD_DEBUG] ObjectId lookup result: {user is not None}"
            )
        except Exception as e:
        ...
        if user is None:
            ...
            auth_log(
                f"[CHANGE_PASSWORD_DEBUG] String lookup result: {user is not None}"
            )
        ...
        auth_log(
            f"[CHANGE_PASSWORD_DEBUG] Found user: {user.get('_id')} with email: {user.get('email')}"
        )

        # Verify old password with comprehensive format support
        old_password_valid = False
        auth_log(
            f"[CHANGE_PASSWORD_DEBUG] Starting password verification for user {current_user}"
        )
        ...
        if "password_hash" in user and "password_salt" in user:
            try:
                old_password_valid = verify_password(
                    old_password, user["password_hash"], user["password_salt"]
                )
                auth_log(
                    f"[CHANGE_PASSWORD_DEBUG] New format verification: {old_password_valid}"
                )
            except Exception as e:
        ...
        if not old_password_valid and "password" in user:
            try:
                ...
                auth_log(
                    f"[CHANGE_PASSWORD_DEBUG] Legacy format verification: {old_password_valid}"
                )
            except Exception as e:
        ...
        if not old_password_valid and "password" in user:
            try:
                ...
                if (
                    ...
                    and "$" in password_str
                    and len(password_str.split("$")) == 2
                ):
                    salt, stored_hash = password_str.split("$", 1)
                    if len(salt) == 32 and len(stored_hash) == 64:  # Validate format
                        old_password_valid = verify_password(
                            old_password, stored_hash, salt
                        )
                        auth_log(
                            f"[CHANGE_PASSWORD_DEBUG] Alternative legacy format verification: {old_password_valid}"
                        )
                    else:
                        auth_log(
                            f"[CHANGE_PASSWORD_DEBUG] Invalid legacy format: salt_len={len(salt)}, hash_len={len(stored_hash)}"
                        )
            except Exception as e:
                auth_log(
                    f"[CHANGE_PASSWORD_DEBUG] Alternative legacy format failed: {e}"
                )

        # Final validation
        if not old_password_valid:
            auth_log(
                f"[CHANGE_PASSWORD_FAILED] Invalid old password for user {current_user}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        auth_log(
            f"[CHANGE_PASSWORD_DEBUG] Using user_id_for_update: {user_id_for_update} (type: {type(user_id_for_update)})"
        )
        ...
        try:
            ...
            auth_log(
                f"[CHANGE_PASSWORD_DEBUG] Password update result: matched={update_result.matched_count}, modified={update_result.modified_count}"
            )
        except Exception as e:
        ...
        try:
            ...
            auth_log(
                f"[CHANGE_PASSWORD_SUCCESS] Tokens invalidated for user {current_user}: matched={invalidate_result.matched_count}, modified={invalidate_result.modified_count}"
            )
        except Exception as e:
...
async def generate_qr_code(
    request: QRCodeRequest, current_user: str = Depends(get_current_user)
):
...
async def verify_qr_code(
    request: VerifyQRCodeRequest, current_user: str = Depends(get_current_user)
):
    ...
    try:
        ...
        if result["success"]:
            ...
            return VerifyQRCodeResponse(success=False, message=result["error"])
...
async def list_qr_code_sessions(current_user: str = Depends(get_current_user)):
...
@router.get("/devices")
async def get_linked_devices(current_user: str = Depends(get_current_user)):
...
async def revoke_device(device_id: str, current_user: str = Depends(get_current_user)):
...
async def register_device_crypto(current_user: str = Depends(get_current_user)):
...
async def get_device_bundle(
    user_id: str, device_id: str, current_user: str = Depends(get_current_user)
):
    ...
    try:
        ...
        if not bundle:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Device bundle not found"
            )
...
async def rotate_crypto_keys(current_user: str = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            auth_log(
                f"Note: Crypto services may not be fully initialized for key rotation"
            )
        except Exception as e:
