...
async def upload_group_avatar(
    ...
    try:
        ...
        for member_id in group.get('members', []):
            if member_id != current_user:  # Don't send to the uploader
                await websocket_manager.send_message_to_device(member_id, broadcast_message)
        
        logger.info(f"Group avatar updated and broadcast: {group_id} by {current_user}")
        
    except Exception as e:
        logger.warning(f"Failed to broadcast group avatar update: {e}")
        # Continue without broadcast - the avatar is still updated
