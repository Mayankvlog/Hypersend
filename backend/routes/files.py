...
async def initialize_upload(
    ...
    try:
        ...
        # Store upload data in database with proper schema
        upload_data = {
            "_id": upload_id,  # Use upload_id as _id for direct lookup
            ...
            "s3_uploaded": False,   # Track S3 upload status
...
async def complete_upload(
    ...
    try:
        ...
        # Find upload record in MongoDB using _id (since we store upload_id as _id)
        upload_record = await uploads_collection().find_one({"_id": upload_id})
        ...
        try:
            ...
            # 🔍 DEBUG: Log complete upload flow
            _log("info", f"🔍 UPLOAD COMPLETE DEBUG", {
                "upload_id": upload_id,
                "current_user": current_user,
                "s3_key": s3_key,
                "mongo_file_id": str(file_id),
                "step": "complete_upload_api"
            })
            
            _log("info", f"✅ File record inserted into files collection: {file_id} for upload_id: {upload_id}")
            _log("info", f"🔗 MongoDB ↔ S3 mapping: file_id={file_id} → s3_key={s3_key}")
        ...
        # Update uploads collection with completion status - CRITICAL FIX
        await uploads_collection().update_one(
            {"_id": upload_id},  # Use _id since we store upload_id as _id
            {
                "$set": {
                    "status": "completed",
                    "s3_uploaded": True,  # CRITICAL: Mark S3 upload as complete
                    "file_url": file_url,
