...
class MessageFanOutWorker:
    ...
    def __init__(self, redis_client: Optional[Any] = None):
        ...
        if hasattr(redis_client, "cluster"):
            logger.warning(
                "Detected Redis cluster client, switching to standalone mode"
            )
            ...
            if connection_pool and hasattr(connection_pool, "connection_kwargs"):
                has_password = (
                    connection_pool.connection_kwargs.get("password") is not None
                )
        ...
        try:
            ...
            logger.warning(f"Failed to initialize media_service: {e}")
            self.media_service = None
        try:
            self.signal_protocol = SignalProtocol(redis_client)
        except Exception as e:
            logger.warning(f"Failed to initialize signal_protocol: {e}")
        ...
        while True:
            try:
                ...
                if "authentication required" in error_msg or "noauth" in error_msg:
                    logger.error(
                        f"Redis authentication failed in fan-out worker main loop: {e}"
                    )
                    # Wait much longer for authentication issues
                    await asyncio.sleep(60)
                elif "connection" in error_msg or "redis" in error_msg:
                    logger.error(
                        f"Redis connection error in fan-out worker main loop: {e}"
                    )
    ...
    async def _process_priority_queue(self, priority: str):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"LMPOP not available or failed: {lmpop_error}, using LPOP loop"
                )
                tasks = []
    ...
    async def _process_task_with_semaphore(
        self, semaphore: asyncio.Semaphore, task: FanOutTask
    ):
    ...
    async def _process_single_task(self, task: FanOutTask):
        ...
        try:
            logger.info(
                f"Processing fan-out task {task.task_id} for message {task.message_id}"
            )
    ...
    async def _get_recipient_devices(self, task: FanOutTask) -> List[str]:
        ...
        if not self.device_manager:
            logger.warning("device_manager not available")
            return devices
        ...
        for member_id in members:
            ...
            try:
                ...
                logger.warning(f"Failed to get devices for member {member_id}: {e}")
    ...
    async def _deliver_to_device(self, task: FanOutTask, device_id: str) -> bool:
        ...
        try:
            ...
            encrypted_message = await self._encrypt_message_for_device(
                task.message_data, device_session
            )
    ...
    async def _encrypt_message_for_device(
        self, message_data: Dict[str, any], device_session
    ) -> Dict[str, any]:
        ...
        encrypted_content = await self.signal_protocol.encrypt_message(
            message_data["content"], device_session.session_key
        )
    ...
    async def _send_to_device_websocket(self, device_id: str, message: Dict[str, any]):
        ...
        await self.redis.lpush(f"device_outbox:{device_id}", json.dumps(message))
    ...
    async def _store_offline_message(self, task: FanOutTask, device_id: str):
        ...
        await self.redis.lpush(
            f"device_offline_queue:{device_id}", json.dumps(task.message_data)
        )
    ...
    async def _handle_successful_deliveries(self, task: FanOutTask, devices: List[str]):
        ...
        for device_id in devices:
            logger.info(
                f"Successfully delivered message {task.message_id} to device {device_id}"
            )
    ...
    async def _handle_failed_deliveries(self, task: FanOutTask, devices: List[str]):
        ...
        for device_id in devices:
            logger.warning(
                f"Failed to deliver message {task.message_id} to device {device_id}"
            )
    ...
    async def _mark_task_completed(self, task: FanOutTask):
        ...
        await self.redis.hset(f"fanout_task:{task.task_id}", "status", "completed")
        await self.redis.hset(
            f"fanout_task:{task.task_id}", "completed_at", time.time()
        )
    ...
    async def _mark_task_failed(self, task: FanOutTask):
        """Mark task as failed"""
        await self.redis.hset(f"fanout_task:{task.task_id}", "status", "failed")
        await self.redis.hset(f"fanout_task:{task.task_id}", "failed_at", time.time())
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(
        3600.0, cleanup_expired_tasks.s(), name="cleanup-expired-tasks"
    )
