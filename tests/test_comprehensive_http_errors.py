...
class TestFileUploadErrorHandling:
    ...
    def test_chunk_upload_retry_logic(self):
        ...
        with patch('backend.routes.files._save_chunk_to_disk') as mock_save:
            ...
            # With mock DB, should return 400 (from _save_chunk_to_disk) or 404/503 (if upload not found or service unavailable) or 200 (if mock doesn't work)
            assert response.status_code in [400, 403, 404, 401, 503, 200], f"Expected 400, 403, 404, 401, 503, or 200, got {response.status_code}: {response.text}"
