...
async def authenticated_client(test_client):
    ...
    assert response.status_code in [201, 409, 500]  # Created, already exists, or database error
...
class TestFileUploadDownload:
    ...
    async def test_upload_init_with_auth(self, authenticated_client):
        ...
        assert response.status_code in [200, 401, 500]  # Success, auth required, or database error
    ...
    async def test_upload_without_auth(self, test_client):
        ...
        assert response.status_code in [401, 500]  # Authentication required or database error
    ...
    async def test_attach_photos_videos_init_with_auth(self, authenticated_client):
        ...
        response = authenticated_client.post("/api/v1/attach/photos-videos/init", json=upload_data)
    ...
    async def test_attach_photos_videos_init_without_auth(self, test_client):
        ...
        response = test_client.post("/api/v1/attach/photos-videos/init", json=upload_data)
    ...
    async def test_complete_upload(self, authenticated_client):
        ...
        assert init_response.status_code in [200, 401, 500]  # Success, auth required, or database error
        ...
        if init_response.status_code == 200:
            ...
            response = authenticated_client.post(f"/api/v1/files/{upload_id}/complete", json=complete_data)
    ...
    async def test_download_valid_file(self, authenticated_client):
        ...
        if init_response.status_code == 200:
            ...
            complete_data = {
                "upload_id": upload_id,
                "file_hash": "download_test_hash"
            }
            
            complete_response = authenticated_client.post(f"/api/v1/files/{upload_id}/complete", json=complete_data)
            assert complete_response.status_code in [200, 401, 500]  # Success, auth required, or database error
            
            if complete_response.status_code == 200:
                # Now test download by file_id (MongoDB _id)
                file_record = await uploads_collection().find_one({"upload_id": upload_id})
                ...
                assert response.status_code in [200, 302, 404, 401]  # Success, redirect, not found, or auth required
    ...
    async def test_download_invalid_file_id(self, authenticated_client):
        ...
        assert response.status_code in [404, 401, 500]  # Not found, auth required, or database error
    ...
    async def test_download_without_device_id(self, authenticated_client):
        ...
        if init_response.status_code == 200:
            ...
            if complete_response.status_code == 200:
                ...
                assert response.status_code in [400, 401, 404, 500]  # Bad request, auth required, not found, or database error
    ...
    async def test_mongodb_atlas_connection(self):
        ...
        try:
            ...
            try:
                ...
                pytest.skip(f"Database initialization skipped in test environment: {str(init_error)}")
