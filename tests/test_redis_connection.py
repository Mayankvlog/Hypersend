...
class TestRedisConnection:
    ...
    async def test_redis_connection_success(self):
        ...
        with patch("redis.asyncio.Redis", return_value=mock_redis_client) as mock_redis:
            with patch("backend.main.settings") as mock_settings:
                ...
                assert (
                    mock_redis_client.info.call_count == 2
                ), "info should be called twice (server + memory)"
    ...
    async def test_redis_connection_retry_mechanism(self):
        ...
        # Mock redis.asyncio module
        with patch(
            "redis.asyncio.Redis", side_effect=create_redis_client
        ) as mock_redis:
            with patch("backend.main.settings") as mock_settings:
                mock_settings.REDIS_HOST = "redis"
                mock_settings.REDIS_PORT = 6379
    ...
    async def test_redis_connection_all_retries_fail(self):
        ...
        with patch("redis.asyncio.Redis", side_effect=create_redis_client):
            with patch("backend.main.settings") as mock_settings:
                ...
                with pytest.raises(
                    RuntimeError, match="Redis connection failed after 5 attempts"
                ):
...
class TestWebSocketManagerRedisIntegration:
    ...
    async def test_websocket_manager_fails_without_redis(self):
        """Test WebSocket manager runs in degraded mode without Redis client"""
        ...
        # Mock the test mode detection to force non-test mode
        with patch("database._is_pytest_running", return_value=False):
            # Should NOT raise RuntimeError - instead runs in degraded mode
            # The app should start successfully with mock cache
            try:
                with TestClient(app) as client:
                    ...
                    print(
                        "[OK] App started successfully in degraded mode without Redis"
                    )
            except RuntimeError as e:
                # If RuntimeError is raised, check if it's an expected graceful degradation message
                error_msg = str(e)
                if any(
                    ...
                    # This is acceptable - graceful degradation message
                    print(f"[OK] Graceful degradation: {error_msg}")
                else:
                    # Unexpected error
                    raise
