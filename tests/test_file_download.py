...
class TestFileDownload:
    ...
    def test_api_service_download_small_file(self):
        ...
        if file_size <= 100 * 1024 * 1024:  # Small file
            self.mock_api_service.downloadFileToPathWithProgress(
                fileId=self.test_file_id, savePath="test_path", onProgress=Mock()
            )
    ...
    def test_api_service_download_large_file(self):
        ...
        if file_size > 100 * 1024 * 1024:  # Large file
            self.mock_api_service.downloadLargeFileToPath(
                fileId=self.test_file_id, savePath="test_path", onReceiveProgress=Mock()
            )
    ...
    def test_file_download_error_handling(self):
        ...
        self.mock_api_service.downloadFileToPathWithProgress.side_effect = Exception(
            "Permission denied"
        )
    ...
    def test_file_verification_after_download(self):
        ...
        with patch("os.path.exists", return_value=False):
            ...
            with pytest.raises(
                Exception, match="File download completed but file not found"
            ):
                if not os.path.exists(file_path):
                    raise Exception(
                        f"File download completed but file not found at path: {file_path}"
                    )
...
class TestFileDownloadAuthentication:
        ...
        for invalid_token in invalid_tokens:
            ...
            try:
                ...
                decoded = pyjwt.decode(
                    invalid_token, options={"verify_signature": False}
                )
    ...
    def test_authorization_header_format_validation(self):
        ...
        for header_value, should_be_valid in auth_headers:
            if header_value:
                is_valid = (
                    header_value.lower().startswith("bearer ")
                    and len(header_value.split(" ", 1)) == 2
                )
            else:
                is_valid = False
...
class TestFileDownloadUIFix:
    ...
    def test_download_dialog_removed_spinner(self):
        ...
        download_method_start = content.find(
            "Future<void> _downloadFile(Message message)"
        )
        assert download_method_start > 0, "_downloadFile method should exist"

        # Find the _proceedWithDownload method (the actual download dialog, not image preview)
        proceed_method_start = content.find(
            "Future<void> _proceedWithDownload(", download_method_start
        )
        assert proceed_method_start > 0, "_proceedWithDownload method should exist"

        # Find the showDialog call within _proceedWithDownload (this is the download dialog)
        dialog_start = content.find("showDialog(", proceed_method_start)
        assert dialog_start > 0, "showDialog should be called in _proceedWithDownload"

        # Implement brace-counting scan to find the complete dialog closure
        brace_count = 0
        in_string = False
        escape_next = False
        dialog_end = dialog_start
        
        for i, char in enumerate(content[dialog_start:], dialog_start):
            if escape_next:
                escape_next = False
                continue
        ...
        assert dialog_end > dialog_start, "Could not find complete dialog closure"
        
        # Get the complete dialog content (from showDialog to the closing brace)
        dialog_section = content[dialog_start : dialog_end + 1]
        ...
        assert not has_spinner_in_dialog, f"Download dialog should not contain CircularProgressIndicator. Found in dialog: {dialog_section}"
