        ...
        if "message" in data and "data" in data:
            ...
            if isinstance(data["detail"], str):
                ...
                try:
                    ...
                    if "data" in nested_data and "error_code" in nested_data["data"]:
                        assert nested_data["data"]["error_code"] == "MISSING_CONTENT_TYPE"
                except:
                    print(f"INFO: Could not parse nested error: {data['detail']}")
            else:
                print(f"INFO: Error format (detail): {data}")
        ...
        if "message" in data and "data" in data:
            ...
            if isinstance(data["detail"], str):
                ...
                try:
                    ...
                    if "data" in nested_data and "error_code" in nested_data["data"]:
                        assert nested_data["data"]["error_code"] == "PRESIGN_VALIDATION_ERROR"
                except:
                    print(f"INFO: Could not parse nested error: {data['detail']}")
            else:
                print(f"INFO: Error format (detail): {data}")
            ...
            if "message" in data and "data" in data:
                ...
                if isinstance(data["detail"], str):
                    ...
                    try:
                        nested_data = json.loads(data["detail"])
                        if "message" in nested_data:
                            assert payload_data["expected_error"].lower() in nested_data["message"].lower()
                    except:
                        print(f"INFO: Could not parse nested error: {data['detail']}")
                else:
                    print(f"INFO: Error format (detail): {data}")
            ...
            if "message" in data and "data" in data:
                ...
                if isinstance(data["detail"], str):
                    ...
                    try:
                        nested_data = json.loads(data["detail"])
                        if "message" in nested_data:
                            assert "unsupported" in nested_data["message"].lower() or "dangerous" in nested_data["message"].lower()
                    except:
                        print(f"INFO: Could not parse nested error: {data['detail']}")
                else:
                    print(f"INFO: Error format (detail): {data}")
