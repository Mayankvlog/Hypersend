...
def validate_imports():
    ...
    try:
        ...
        print("[PASS] All functions imported successfully from auth.utils")
        
        from routes.auth import get_current_user_from_query as auth_import
        print("[PASS] get_current_user_from_query imported in routes.auth")
...
def check_function_signatures():
    ...
    if len(params1) != 1:
        print(f"[FAIL] FAILED: get_current_user has {len(params1)} params, expected 1")
        return False
    
    param1 = params1[0]
    if param1.name != 'credentials':
        print(f"[FAIL] FAILED: get_current_user param is '{param1.name}', expected 'credentials'")
        return False
    
    print("[PASS] get_current_user has correct signature (credentials: HTTPAuthorizationCredentials = Depends(security))")
    ...
    if len(params2) != 1:
        print(f"[FAIL] FAILED: get_current_user_from_query has {len(params2)} params, expected 1")
        return False
    
    param2 = params2[0]
    if param2.name != 'token':
        print(f"[FAIL] FAILED: get_current_user_from_query param is '{param2.name}', expected 'token'")
        return False
    
    # Check that it uses Query
    if 'Query' not in str(param2.default.__class__.__name__):
        print(f"[FAIL] FAILED: get_current_user_from_query doesn't use Query() - uses {param2.default.__class__.__name__}")
        return False
    
    print("[PASS] get_current_user_from_query has correct signature (token: Optional[str] = Query(None))")
...
def validate_all_route_usages():
    ...
    for route_file in route_files:
        if not os.path.exists(route_file):
            print(f"[SKIP] {route_file} - NOT FOUND (skipped)")
            continue
        ...
        if 'get_current_user' in content:
            ...
            if re.search(pattern, content):
                ...
                print(f"[PASS] {route_file}: {usage_count} correct usage(s) of get_current_user")
            else:
                # Check for bare usage (which would be wrong)
                if re.search(r':\s*get_current_user\b', content):
                    print(f"[FAIL] {route_file}: Found bare get_current_user usage (missing Depends)")
                    issues.append(f"{route_file}: bare get_current_user")
                    all_valid = False
    
    if all_valid:
        print("\n[PASS] All route usages are valid")
    else:
        print(f"\n[FAIL] Found {len(issues)} usage issue(s)")
        for issue in issues:
            print(f"  - {issue}")
...
def validate_query_parameter_usage():
    ...
    if 'from fastapi import' in utils_content and 'Query' in utils_content:
        print("[PASS] Query is imported in backend/auth/utils.py")
    else:
        print("[FAIL] FAILED: Query not imported in backend/auth/utils.py")
        return False
    
    # Check for Query usage in get_current_user_from_query
    if 'Query(None)' in utils_content:
        print("[PASS] Query(None) is used in get_current_user_from_query")
    else:
        print("[FAIL] FAILED: Query(None) not found in get_current_user_from_query")
        return False
...
def validate_no_breaking_changes():
    ...
    async def test_header_auth():
        try:
            ...
            if result == user_id:
                print("[PASS] get_current_user still works with Bearer token (no breaking changes)")
                return True
            else:
                print(f"[FAIL] FAILED: get_current_user returned '{result}', expected '{user_id}'")
                return False
        except Exception as e:
            print(f"[FAIL] FAILED: get_current_user error: {type(e).__name__}: {e}")
            return False
...
def main():
    ...
    for name, check_func in checks:
        try:
            ...
            print(f"\n[FAIL] ERROR in {name}: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
    ...
    if passed == total:
        print("\n[PASS] ALL VALIDATIONS PASSED - NO BREAKING CHANGES DETECTED")
        return True
    else:
