...
async def test_upload_completion_fix():
    """Test upload completion fix"""
    print("🔧 TESTING UPLOAD COMPLETION FIX")
    print("=" * 50)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        register_data = {
            "email": f"uploadfix_{datetime.now().timestamp()}@example.com",
            "password": "TestPassword123",
            "full_name": "Upload Fix User"
        }
        ...
        async with session.post(f"{BASE_URL}/api/v1/files/{upload_id}/complete", headers=headers) as resp:
            ...
            if resp.status in [200, 201]:
                ...
                if resp.status == 500:
                    print("🔍 500 Error - This is progress from 403!")
                    print("🔍 The ownership fix worked")
                    print("🔍 Now we need to fix the internal server error")
                
                return False

if __name__ == "__main__":
    result = asyncio.run(test_upload_completion_fix())
    ...
    if result:
        ...
        print("\n❌ 404 ERROR FIX IN PROGRESS")
        print("📌 Ownership validation fixed (403 → 500)")
        print("📌 Need to fix internal server error")
